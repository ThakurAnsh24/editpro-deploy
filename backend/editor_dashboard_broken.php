        ...
        if ($check->get_result()->num_rows > 0) {
            ...
            if (isset($_FILES['completed_file']) && $_FILES['completed_file']['error'] === 0) {
                ...
                if (move_uploaded_file($_FILES['completed_file']['tmp_name'], $target_file)) {
                    ...
                    if ($update) {
                        $update->bind_param("sssi", $status ?: 'Ready for Review', $notes, $file_name, $order_id);
                        $update->execute();
                        $update->close();
                    }
                }
                ...
                if ($update) {
                    $update->bind_param("ssi", $status, $notes, $order_id);
                    $update->execute();
                    $update->close();
                }
            }
...
<body>
                ...
                <?php foreach ($orders as $order): ?>
                    <div class="order-card">
                        ...
                        <div style="margin-top: 1rem;">
                            <button class="btn btn-primary" onclick="openForm(<?= $order['id'] ?>)">✏️ Update</button>
                            <button class="btn btn-warning" onclick="downloadPreview(<?= $order['id'] ?>)">📥 Download</button>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
    ...
    <script>
        ...
        function downloadPreview(id) {
            window.location.href = 'download_file.php?id=' + id;
                            ...
                            <div class="timer">
                                <?php 
                                $remaining = $order['remaining_seconds'] ?? 0;
                                if ($remaining > 0): ?>
                                    <span class="timer-running">⏱️ <?= floor($remaining / 60) ?>m</span>
                                <?php else: ?>
                                    <span class="timer-finished">✅ Complete</span>
    ...
    <!-- Update Form Overlay -->
    <div id="updateForm" class="form-overlay" style="display: none;">
        <div class="form-card">
            <button class="close-btn" onclick="closeForm()">×</button>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <textarea name="editor_notes" placeholder="Any notes for admin..."></textarea>
                </div>
                <div class="form-group">
                    <label>Upload Completed File (Optional)</label>
                    <div class="file-input-wrapper">
                        <input type="file" name="completed_file" accept="image/*,video/*">
                        <label>Choose File</label>
                    </div>
                </div>
